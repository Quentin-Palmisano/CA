...
class MLarray extends MLvalue {
  private ArrayList<MLvalue> MLtab;

  MLarray(){MLtab = new ArrayList<MLvalue>();}

  public MLvalue MLaccess1(int i){return MLtab.get(i);}
  public void MLset(MLvalue a){MLtab.add(a);}
  public void MLset(int i, MLvalue a){MLtab.set(i, a);}
  public int MLlength(){return MLtab.size();}

  public void print(){if (MLtab.size() == 0) {System.out.print("[||]");}
...
class MLprimitive extends MLfun {
  ...
  public MLvalue invoke(MLvalue l) {
    ...
    else if (name.equals("length")) return MLruntime.MLlength_real((MLarray)l);
...
class MLruntime {
  ...
  // array
  public static MLarray MLmake(MLint l, MLvalue x) {
        MLarray tab = new MLarray();
        for(int i=0; i < l.MLaccess();i++) {tab.MLset(x);}
        return tab;
  }
  public static MLvalue MLget(MLarray tab ,MLint i){
    return  tab.MLaccess1(i.MLaccess());
  }
  public static void MLset(MLarray tab ,MLint i, MLvalue a){
    tab.MLset(i.MLaccess(), a);
  }
  ...
  // acces a la longueur d'un tableau
  public static MLvalue MLlength = new MLprimitive("length");
  public static MLint MLlength_real(MLarray tab) {
    return new MLint(tab.MLlength());
  }
