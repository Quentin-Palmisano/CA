...
bool Instruction::is_dep_WAR1(Instruction *i2){
  return ((i2->get_reg_dst() != nullptr && 
	  ...
	  || (i2->is_call() &&  this->get_reg_src1() != nullptr && this->get_reg_src1()->get_reg_num() == 31)	  
	  );
}
...
bool Instruction::is_dep_WAR2(Instruction *i2){
  return  ((i2->get_reg_dst() != nullptr &&
	   ...
	   || (i2->is_call() &&  this->get_reg_src2() != nullptr && this->get_reg_src2()->get_reg_num() == 31)
	   );
}
...
bool Instruction::is_dep_WAW(Instruction *i2){
  return (
	  (i2->get_reg_dst() != nullptr && get_reg_dst()!= nullptr &&  
	  !(i2->get_reg_dst()->get_op().compare(this->get_reg_dst()->get_op())))
	  || (i2->is_call() && this->get_reg_dst() != nullptr && this->get_reg_dst()->get_reg_num() == 31)) ;
}
...
bool Instruction::is_dep_MEM(Instruction *i2){
  ...
  if(!( this->get_type()==t_Inst::MEM 
    ...
    return false;
  if (is_mem_load() && i2->is_mem_load())
    return false;
  //  return true;

  /* if ( ((is_mem_load() && get_reg_src1()->get_reg_num() == 29) 
  ...
  if ( (is_mem_load() && i2->is_mem_load() && get_reg_src1()->get_reg_num() == i2->get_reg_src1()->get_reg_num()) ||
    ...
    {
      int imm1 = get_immediate() -> get_op_val();
      int imm2 = i2->get_immediate() -> get_op_val();
...
bool Instruction::is_strict_dep_MEM(Instruction *i2){
...
t_Dep Instruction::is_dependent(Instruction *i2){
...
OPImmediate* Instruction::get_immediate(){
  OPImmediate *op1 = dynamic_cast< OPImmediate * > (_op1);
  OPImmediate *op2 = dynamic_cast< OPImmediate * > (_op2);
  OPImmediate *op3 = dynamic_cast< OPImmediate * > (_op3);
  if (op1 != nullptr) return op1;
  if (op2 != nullptr) return op2;
  if (op3 != nullptr) return op3;
  return nullptr;
}
...
OPImmediate* Instruction::get_op_imm(){
  OPImmediate *op1 = dynamic_cast< OPImmediate * > (_op1);
  OPImmediate *op3 = dynamic_cast< OPImmediate * > (_op3);
  OPImmediate *op2 = dynamic_cast< OPImmediate * > (_op2);
...
bool Instruction::is_call(){
  return (_op == t_Operator::jal || is_syscall());
}
...
bool Instruction::is_syscall(){
  return (get_content().compare(0,7,"syscall")==0);
}
